...
const client = new Client({
    intents: [Intents.FLAGS.GUILDS, Intents.FLAGS.GUILD_MESSAGES, Intents.FLAGS.GUILD_MEMBERS, Intents.FLAGS.GUILD_PRESENCES, Intents.FLAGS.GUILD_INTEGRATIONS]
});
...
client.on('message', msg => {
    if (msg.channel.name == "verification" && msg.content != "verify")
        msg.delete();
            ...
            if (!msg.member.roles.cache.some(role => role == verifiedRole)) {
                ABS.verify(msg.author, msg.member).then((score) => {
                    ...
                    if (trusted) {
                        ...
                        msg.author.send("<@" + msg.member.id + ">" + "! Our AntiBot-system has evaluated your account as suspicious. You will not be granted access to this server. If you think this is a mistake, please contact a moderator.").then(function () {
